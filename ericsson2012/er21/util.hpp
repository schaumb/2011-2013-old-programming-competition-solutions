...
struct ComplRoute
{
	...
	std::vector<Point> toShortedPoints()
	{
		std::vector<Point> pnts;
		std::sort(__r[0].begin(), __r[0].end());
	  for(unsigned i=0;i<cr.__r.size();++i){
	   for(unsigned j=0;j<cr.__r[i].size(); ++j){
		pnts.push_back(cr.__r[i][j].pont);
	   }
	  }
  }
};
